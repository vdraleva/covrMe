...
{
    public class SirmaVehicleService : ISirmaVehicleService
    {
        private readonly ILogger<SirmaVehicleService> _logger;
        ...
        public SirmaVehicleService(ILogger<SirmaVehicleService> logger, IConfiguration configuration, ISirmaAuthenticationService sirmaAuthentication,
        ...
        public async Task<GetVehicleModelsResultModel> GetVehicleModelsByBrandId(string brandId)
        {
            var result = new GetVehicleModelsResultModel();

            var url = $"{this._baseUrl}{GlobalConstants.GetVehicleModels}?vehicle_mark_id={brandId}";
            ...
            try
            {
                using (client)
                {
                    ...
                    using (var response = await client.GetAsync(url))
                    {
                        ...
                        {
                            result = JsonConvert.DeserializeObject<GetVehicleModelsResultModel>(apiResponse);
                            if (result.Success == 1)
                            {
                                return result;
                            }
