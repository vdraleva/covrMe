...
{
    public partial class HealthInsuranceDocumentsPageViewModel : BaseViewModel, IQueryAttributable
    {
        #region Fields
        ...
        private async void GoToTermsInsurance(object obj)
        {
            try
            {
                ...
                IsBusy = true;
                ShowLoading();
                string uri = GlobalConstants.ObshtiUsloviaHealthURL;
        ...
        private async void GoToInfoDocument(object obj)
        {
            try
            {
                ...
                IsBusy = true;
                ShowLoading();
                string uri = GlobalConstants.InfoDocumentHealthURL;
                await OpenBrowser(uri);
            }
            catch (Exception ex)
            {
        ...
        private async void GoToPredgovorInfo(object obj)
        {
            try
            {
                ...
                IsBusy = true;
                ShowLoading();
                string uri = GlobalConstants.PreddogovorenInfoHealthURL;
                await OpenBrowser(uri);
            }
            catch (Exception ex)
            {
        ...
        [RelayCommand]
        private async void GoToPrivacyPolicyHealthInfo(object obj)
        {
            try
            {
                if (IsBusy)
                ...
                IsBusy = true;
                ShowLoading();
                string uri = GlobalConstants.PrivacyPolicyHealthURL;
                await OpenBrowser(uri);
            }
            catch (Exception ex)
            {
        ...
        [RelayCommand]
        private async void GoToOperationSchema(object obj)
        {
            try
            {
                if (IsBusy)
                ...
                IsBusy = true;
                ShowLoading();
                string uri = GlobalConstants.OperationSchema;
                await OpenBrowser(uri);
            }
            catch (Exception ex)
            {
